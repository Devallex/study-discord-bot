...
public class CardSubhandler {
    ...
    public InteractionCallbackAction<?> handleCardUpdate(SlashCommandInteractionEvent event) {
        final String cardID = event.getOption("name").getAsString();
        final Flashcard card = Flashcard.acquire(data, cardID);
        ...
        return event.replyModal(
                ...
                Modal.create("flashcard-card-update:" + cardID, "New Flashcard Card")
                        .addComponents(
                                Label.of("Operation",
    ...
    public void handleModal(ModalInteractionEvent event) {
        ...
        if (modalID.startsWith("flashcard-card-new:")) {
            ...
            // FlashcardDeck user = FlashcardDeck.acquire(data, );

            // user.removeCardID(cardID);
            // card.delete();
            // data.push(user, card);

            // event.reply(String.format("Deleted card **%s**", card.getName())).queue();
            // return;
            // }

            // card.setName(event.getValue("name").getAsString());
            // card.setDescription(event.getValue("description").getAsOptionalString());

            // data.push(card);
            // event.reply(String.format("Updated card **%s**", card.getName())).queue();
            // ;
        }
    }
    // endregion
